use egui::{Ui, RichText, Color32, Stroke, Pos2, Rect, Response};
use egui_plot::{Plot, PlotPoints, Line, Points, Legend};
...
impl FanCurveEditor {
    ...
    fn draw_reference_zones(&self, plot_ui: &mut egui_plot::PlotUi) {
        use egui_plot::{Polygon, PlotPoint};
    ...
    fn draw_points_editor(&mut self, ui: &mut Ui) {
        ...
        egui::Grid::new(format!("points_grid_{}", self.fan_id))
            ...
            .show(ui, |ui| {
                ...
                let mut points_to_update = Vec::new();
                ...
                for (idx, (temp, speed)) in self.curve.points.iter().enumerate() {
                    ...
                    if ui.add(egui::Slider::new(&mut temp_val, 0.0..=100.0)
                        ...
                        .changed() 
                    {
                        points_to_update.push((idx, temp_val as u8, *speed));
                    }
                    ...
                    if ui.add(egui::Slider::new(&mut speed_val, 0.0..=100.0)
                        ...
                        .changed() 
                    {
                        points_to_update.push((idx, *temp, speed_val as u8));
                    }
                ...
                // Apply updates
                for (idx, temp, speed) in points_to_update {
    ...
    fn add_point(&mut self) {
        ...
        // Find gap in temperature range to insert point
        let mut sorted = self.curve.points.clone();
        sorted.sort_by_key(|p| p.0);
        ...
        if 100 - sorted.last().unwrap().0 > best_gap_size {
            best_gap_temp = sorted.last().unwrap().0 + (100 - sorted.last().unwrap().0) / 2;
        }
        
        // Interpolate speed at this temperature
